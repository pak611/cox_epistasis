...
#' simdata$beta
generate.lm <- function(baseline, X=NULL, N=1000, type="none", beta=NULL, xvars=3, mu=0, sd=1, censor=.1, interactions=FALSE, inter.mat=NULL){
     T <- max(baseline$time)
     if(type=="none"){
          ...
          if (interactions==TRUE){
               calc.interactions <- function(inst.vals, inter.mat){
                    attr.ind <- 1:xvars
                    # Generate all combinations of attribute indices
                    combs <- expand.grid(attr.ind, attr.ind)
                    prods <- mapply(function(x,y) inst.vals[x]*inst.vals[y], combs[,1], combs[,2])
                    weighted.prods <- inter.mat * prods
